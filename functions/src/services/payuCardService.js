const PayuCard = require('../models/PayuCard');
const createBaseService = require('./base/serviceFactory');
const { NotFoundError, BadRequestError } = require('../utils/errors');

const createPayuCardService = () => {
  const baseService = createBaseService('payuCards', PayuCard, 'bakeries/{bakeryId}');
  ...
  const createCard = async (cardData, bakeryId) => {
    try {
      ...
      if (response.code === 'SUCCESS' && response.creditCardToken) {
        ...
        const saved = await baseService.create(payuCard, bakeryId);
        return saved.toClientObject();
      } else {
        throw new BadRequestError(`Card creation failed: ${response.error || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  // Get all stored payment cards for a bakery
  const getStoredCards = async (bakeryId, query = {}) => {
    try {
      const filter = {
        ...query.filters,
        status: 'ACTIVE',
      ...
      return {
        ...results,
        items: results.items.map(card => card.toClientObject()),
      };
    } catch (error) {
  ...
  const deleteCard = async (cardId, bakeryId) => {
    try {
      // Get the card record by our database ID
      const card = await baseService.getById(cardId, bakeryId);
      ...
      const requestData = {
        ...
        removeCreditCardToken: {
          ...
          creditCardTokenId: card.tokenId, // Use the actual PayU token ID
        },
      };
      ...
      if (response.code === 'SUCCESS') {
        // Mark card as deleted in our database
        await baseService.patch(card.id, {
          status: 'DELETED',
          isDeleted: true,
        }, bakeryId);
        return { success: true, message: 'Card deleted successfully' };
      } else {
        throw new BadRequestError(`Card deletion failed: ${response.error || 'Unknown error'}`);
      }
    } catch (error) {
...
module.exports = createPayuCardService();
